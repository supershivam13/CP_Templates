...
// to store the count of nodes on level 0 or on level 1
int lev[2];

// to store the present ids in the test case
int isthere[100001];
...
void bfs(int src) {
    ...
    vis[src] = 1;
    // queue to store pairs of {node, level (0 or 1)}
    queue<pair<int, int>> q;
    q.push({src, 0});
    // given the src node level 0 and its complementary level to all of its child
    lev[0]++;
    ...
    while (!q.empty()) {
        ...
        // l is the level of the front element of queue
        int l = q.front().second;
        q.pop();

        for (auto child : v[cur]) {
            if (vis[child] == 0 and isthere[child] == 1) {

                // pushing the child in the queue with complementary level of cur
                q.push({child, (l + 1) % 2});
                vis[child] = 1;
    ...
    // to ensure the correct in case of multiple connected components in graph
    // ( keep adding the max possible from each connected component )
    ans = ans + max(lev[0], lev[1]);
    lev[0] = 0;
    lev[1] = 0;
...
int32_t main()
{
    ...
    w(t) {
        ...
        // clearing the visited, adjaceny list and isthere array for the fresh testcase
        fo(i, 1, 20000) {
            vis[i] = 0;
            v[i].clear();
            isthere[i] = 0;
        ...
        // initializing the variables to zero for the fresh testcase
        ans = 0;
        lev[0] = 0;
        lev[1] = 0;
        ...
        // calling the bfs for all the unvisited nodes which are present in the given testcase
        fo(i, 1, 20000)
        if (vis[i] == 0 and isthere[i] == 1)
            bfs(i);
