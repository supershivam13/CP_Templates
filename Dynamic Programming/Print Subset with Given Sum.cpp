...
// TIME COMPLEXITY - O(N*M)    ( where m is the sum to be formed using the subset of array )
...
int32_t main()
{
    ...
    // n =  number of elements, target = sum to be be formed using subset of array
    int n, target;
    cin >> n >> target;
    ...
    // initailising all values as 0
    // Here, dp[i] will store the last element used to make the sum i
    int dp[target + 1];
    fo(i, 1, target)
    dp[i] = 0;

    // zero sum can always be obtained using empty subset
    dp[0] = 1;
