...
// KADANE'S ALGORITHM - FOR FINDING THE MAXIMUM SUBARRAY SUM OF AN ARRAY
...
int32_t main()
{
    ...
    // DP[i] = maximum possible subarray sum ending at position i
    // ( i.e dp[i] = maxisum subarray sum of all subarrays which are ending at position i )
    fo(i, 1, n) {
        // a[i] will always be there, adding the dp[i-1] only if it is greater than 0
        dp[i] = max(dp[i - 1], z) + a[i];
    }

    int result = *max_element(dp + 1, dp + n + 1);

    cout << max(result, z) << endl;
