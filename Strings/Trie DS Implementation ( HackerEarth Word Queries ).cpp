...
// structure of node ( as per the requirements )
struct node {
    int count;
    node* ar[SIZE];
};
...
// Function to insert a word in Trie DS
...
// Function to return the answer according to the question
int getAns(node *root , string st)
{
    // creating a copy of the main root node (as root node is the most important node of Trie)
    node *tempRoot = root;
...
int main()
{
    ...
    while (q--)
        cin >> st , cout << getAns(root , st) << endl;
