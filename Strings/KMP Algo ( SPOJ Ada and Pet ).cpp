...
// globally defined lps array ( to avoid time in creating new array in each test case )
int lps[500005];
...
int32_t main()
{
    w(t) {
        ...
        int overlapping_length = lps[t - 1];

        cout << t + ((k - 1) * (t - overlapping_length)) << "\n";
