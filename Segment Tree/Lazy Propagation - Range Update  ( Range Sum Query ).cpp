...
void build(int s_ind, int ss, int se) {
    ...
    segtree[s_ind] = segtree[2 * s_ind] + segtree[2 * s_ind + 1];

}

// Range Update ( Lazy Propagation )
void update(int s_ind, int ss, int se, int qs, int qe, int val) {

    // for addressing any pending updates before this current update
    if (lazy[s_ind] != 0) {
        int dx = lazy[s_ind];
        lazy[s_ind] = 0;

        segtree[s_ind] += dx * (se - ss + 1);
    ...
    // Now moving on with the current update

    // completely outside
    if (qs > se or qe < ss)
        return;

    // completely inside ( segment tree completely inside the query range )
    if (ss >= qs and se <= qe) {
    ...
    update(2 * s_ind, ss, mid, qs, qe, val);
    update(2 * s_ind + 1, mid + 1, se, qs, qe, val);
...
int query(int s_ind, int ss, int se, int qs, int qe) {
    ...
    return query(2 * s_ind, ss, mid, qs, qe) + query((2 * s_ind) + 1, mid + 1, se, qs, qe);
}
...
int32_t main()
{
    ...
    while (q--) {
        ...
        if (type == 1) {
            // Query is to give the sum in the range L to R
            ...
            // Query is to update the range L to R ( add val to elements in range L to R )
            int qs, qe, val;
            cin >> qs >> qe >> val;
