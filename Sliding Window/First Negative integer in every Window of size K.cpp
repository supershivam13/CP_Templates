...
int32_t main()
{
    ...
    // getting all the negative elements of first window of size k
    // and storing them in a deque
    for (int i = 0; i < k; i++) {
        if (a[i] < 0)
            d.push_back(a[i]);
    }
    ...
    for (int i = 0; i <= n - k; i++) {
        ...
        if (i == 0) {
            ...
            // if no negative element is present in the window
            if (d.empty() == true)
                ans.push_back(0);
            // else insert the front element of the list in the ans vector
            else
