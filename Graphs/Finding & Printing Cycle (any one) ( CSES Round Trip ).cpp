...
// vector to store the cycle
vi path;



// Depth First Search for Cycle Detection (any one cycle) in Graphs
bool dfs(int node, int parent) {
    ...
    for (auto child : v[node]) {
        if (vis[child] == 0) {
            ...
            // when child is already visited and it is not equal to parent
            // then it indicates the presence of a cycle
            if (child != parent) {
                path.pb(child);
                return true;
            }
...
int32_t main()
{
    ...
    // boolean to check the presence of a cycle or not
    bool found = false;


    for (int i = 1; i <= n; i++) {
        if (vis[i] == 0 and dfs(i, -1) == true) {

            // Cycle printing ( from the node A till the node A )
            vi result;

            // start from the last element of the vector and keep printing the elements
            // until the last element appears again
            int l = path.size() - 1;
            int s = path[l];
            result.pb(s);
            l--;

            while (path[l] != s and l >= 0)
            {
                result.pb(path[l]);
                l--;
            }
            ...
            arrout(result.begin(), result.size());

            // so we found a cycle, hence marking as true and breaking the for loop
            found = true;
            break;
        }
